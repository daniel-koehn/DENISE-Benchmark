...
void model_elastic_TTI(float  **  rho, float **  c11, float **  c13, float **  c33, float **  c44, float **  theta){
	...
	float vp03 = 2200.0, vsv3 = 1270.0, rhoh3 = 2000.0, delta3 = 0.70968, epsilon3 = 0.830645, thetah3 = 20.0;
	
	/* 4th layer: anisotropic TTI medium */
	float vp04 = 2800.0, vsv4 = 1620.0, rhoh4 = 2000.0, delta4 = 0.70968, epsilon4 = 0.830645, thetah4 = 45.0;
	...
	/* define amplitude and positions of diffraction points */
	float diff_pert = 0.2;
	int xdiff = floor(NXG/2), ddiff1 = floor(d1/(2.0*DH)), ddiff2 = floor((d1 + (d2-d1)/(2.0)) / DH);
	int ddiff3 = floor((d2 + (d3-d2)/(2.0)) / DH), ddiff4 = floor((d3 + ((NYG*DH)-d3)/(2.0)) / DH);
	int ndiff = 2;
	...
	/* loop over global grid */
		for (i=1;i<=NXG;i++){
			for (j=1;j<=NYG;j++){
				...
				/* add other layers and some diffraction points within */			        
				if(y > d1){vp0 = vp02; vsv = vsv2; rhoh = rhoh2; delta = delta2; epsilon = epsilon2; thetah = thetah2;}
			        if(((j >= ddiff2 - ndiff) && (j <= ddiff2 + ndiff) ) && ((i >= xdiff) && (i < xdiff + ndiff))){vp0 = vp0 + diff_pert * vp0;}
				
				if(y > d2){vp0 = vp03; vsv = vsv3; rhoh = rhoh3; delta = delta3; epsilon = epsilon3; thetah = thetah3;}	
				if(((j >= ddiff3 - ndiff) && (j <= ddiff3 + ndiff) ) && ((i >= xdiff) && (i < xdiff + ndiff))){vp0 = vp0 + diff_pert * vp0;}
				
				if(y > d3){vp0 = vp04; vsv = vsv4; rhoh = rhoh4; delta = delta4; epsilon = epsilon4; thetah = thetah4;}				
				if(((j >= ddiff4 - ndiff) && (j <= ddiff4 + ndiff) ) && ((i >= xdiff) && (i < xdiff + ndiff))){vp0 = vp0 + diff_pert * vp0;}
